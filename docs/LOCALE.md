...
- Duplicate keys: **not allowed**. A key repeated within the same file — including a
  `[section]` header repeated after a different section — is a hard error: Factorio
  fails to load the locale ("Duplicate key ... in property tree at ROOT") and players
  see locale errors. The same applies to duplicate section headers.
- Values may contain `\n` for line breaks (literal backslash-n; the game renders it as
  a newline where newlines are supported).
- Keep order aligned with `en` when adding keys to other locales.
...
7. No duplicate keys — and no duplicate `[section]` headers — within a file.
8. `luacheck .` passes (unrelated to locale, but it's the other CI gate).
9. Locale validation passes: `.temp/redmew-locale/check_locale.ps1` (checks exact
   key set, en key order, duplicate sections/keys, and comment parity — same checks
   the CI pipeline enforces before a PR to `develop` is merged).
